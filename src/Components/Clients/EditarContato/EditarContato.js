...
import { FiX, FiMapPin, FiSave, FiTrash2 } from "react-icons/fi";
...
export default function EditarContato({ onClose, chat }) {
  ...
  useEffect(() => {
    if (chat) {
      const newInitialData = {
        street: chat.street || "", complement: chat.complement || "", number: chat.number || "",
        neighborhood: chat.neighborhood || "", state: chat.state || "", zipcode: chat.zipcode || "",
        country: chat.country || "Brasil", city: chat.city || "", clientName: chat.clientName || "",
      };
      initialData.current = newInitialData;
      setStreet(newInitialData.street); setComplement(newInitialData.complement);
      setNumber(newInitialData.number); setNeighborhood(newInitialData.neighborhood);
      setState(newInitialData.state); setZipcode(newInitialData.zipcode);
      setCountry(newInitialData.country); setCity(newInitialData.city);
      setClientName(newInitialData.clientName);
    }
  }, [chat]);

  useEffect(() => {
    const currentValues = { street, complement, number, neighborhood, state, zipcode, country, city, clientName };
    const hasChanges = Object.keys(currentValues).some((key) => currentValues[key] !== initialData.current[key]);
    setTeveAlteracoes(hasChanges);
  }, [street, complement, number, neighborhood, state, zipcode, country, city, clientName]);
  ...
  const handleSaveEdit = async (e) => {
    ...
    startLoading();
    try {
      await axios.put(`${process.env.REACT_APP_BASE_ROUTE_DOTNET_SERVER}chat/${chat.id}`,
        { ...chat, street, zipcode, number, city, neighborhood, country, clientName, complement, state },
      ...
      <div style={style.modalContainer} onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSaveEdit} style={style.form}>
            <div style={style.inputGroup}>
                ...
                <input value={clientName} onChange={(e) => setClientName(e.target.value)} style={style.input} />
            </div>
            <div style={style.inputGrid}>
                <div style={style.inputGroup}>
                    <label style={style.label}>Contato</label>
                    <input disabled value={func.formatarContato(chat?.id)} style={{...style.input, ...style.disabledInput}} />
                </div>
                <div style={style.inputGroup}>
            ...
            <div style={style.buttonArea}>
                ...
                <button type="submit" style={{...style.actionButton, ...style.saveButton}} disabled={!teveAlteracoes}>
